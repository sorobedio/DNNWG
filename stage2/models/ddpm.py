...
from stage2.modules.util import make_beta_schedule, extract_into_tensor, noise_like
from stage2.models.ddim import DDIMSampler # not needed

""" our code is largely based on the original latent diffusion code 
...
class DDPM(pl.LightningModule):
    ...
    def sample(self, batch_size=16, return_intermediates=False):
        ...
        return self.p_sample_loop((batch_size, channels, input_size, input_size),
...
class LatentDiffusion(DDPM):
    ...
    def progressive_denoising(self, cond, shape, verbose=True, callback=None, quantize_denoised=False,
        ...
        if x_T is None:
            ...
            # img = x_T
            encoder_posterior = self.encode_first_stage(x_T.to(self.device))
            img = self.get_first_stage_encoding(encoder_posterior).detach()
            # img = self.get_first_stage_encoding(x_T.to(self.device))
        intermediates = []
        if cond is not None:
            if isinstance(cond, dict):
    ...
    def p_sample_loop(self, cond, shape, return_intermediates=False,
        ...
        for i in iterator:
            ...
            img = self.p_sample(img, cond, ts,
                                ...
                                quantize_denoised=quantize_denoised)
            if mask is not None:
                x0 = self.get_first_stage_encoding(x0)
                x0 = x0.reshape(self.input_shape)
                img_orig = self.q_sample(x0, ts)
                img = img_orig * mask + (1. - mask) * img
            elif (x0!=None) and (mask==None):
                x0 = self.get_first_stage_encoding(x0)
                x0 = x0.reshape(self.input_shape)
                img = self.q_sample(x0, ts)
    ...
    @torch.no_grad()
    def sample(self, cond, batch_size=5, return_intermediates=False, x_T=None,
               verbose=True, timesteps=None, quantize_denoised=False,
               mask=None, x0=None, shape=None,**kwargs):
        # if len(cond)>batch_size:
        batch_size=len(cond)

        if shape is None:
            shape = (batch_size, self.channels, self.input_size, self.input_size)
        if cond is not None:
            if isinstance(cond, dict):
                cond = {key: cond[key][:batch_size] if not isinstance(cond[key], list) else
                list(map(lambda x: x[:batch_size], cond[key])) for key in cond}
        ...
        if return_intermediates:
            z_q,  iz = self.p_sample_loop(cond,
                                     shape,
                                     return_intermediates=return_intermediates, x_T=x_T,
                                     verbose=verbose, timesteps=timesteps, quantize_denoised=quantize_denoised,
                                     mask=mask, x0=x0)
            x_bar = self.decode_first_stage(z_q)
            xi = [self.decode_first_stage(z).detach().cpu() for z in iz]
            return x_bar, xi
        else:
            z_q = self.p_sample_loop(cond,
                                      shape,
                                      return_intermediates=return_intermediates, x_T=x_T,
                                      verbose=verbose, timesteps=timesteps, quantize_denoised=quantize_denoised,
                                      mask=mask, x0=x0)
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, y=None):
